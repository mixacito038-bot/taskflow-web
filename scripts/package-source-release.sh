...
请先读：docs/部署手册-腾讯云-中国区.md

三种部署方式，任选其一：
  A. 免 Docker（最省事，院内服务器无外网也能装）：bash deploy/install-tencentos.sh
  B. Docker 编排：docker compose -f docker-compose.tencent.yml up -d
  C. 从源码重建：cp .npmrc.china.example .npmrc && npm ci && npm run build

包内已含构建产物 dist/，A 与 B 都不需要 npm install，也不需要访问外网 npm 源。
NOTE
